...
- icon-maskable-512.png (maskable, safe-area-aware)
- apple-touch-icon.png (180x180, fully padded)
...
POM_TOP = (169, 61, 79)        # #A93D4F (pomegranate-400)
POM_BOT = (92, 17, 28)         # #5C111C (pomegranate-700)
PEARL_CYAN = (180, 235, 242)   # #B4EBF2 (subtle crystalline highlight)
WHITE = (255, 255, 255)
...
def _add_crystalline_highlight(canvas, size):
    """Add a soft diagonal glass highlight to evoke crystallinity."""
    overlay = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    od = ImageDraw.Draw(overlay)
    # Upper-left white sheen
    od.polygon([
        (0, 0), (size, 0), (size, int(size * 0.18)),
        (int(size * 0.6), int(size * 0.45)),
        (0, int(size * 0.55)),
    ], fill=(255, 255, 255, 38))
    # Lower-right pearl-cyan tint
    od.polygon([
        (size, size), (size, int(size * 0.45)),
        (int(size * 0.55), size),
    ], fill=(PEARL_CYAN[0], PEARL_CYAN[1], PEARL_CYAN[2], 32))
...
def _draw_logo(canvas, size, full_bleed=False):
    """Draws the brand mark centered on the canvas.

    full_bleed=True for maskable icons: logo is smaller, safe-area aware (80% center).
    """
    ...
    # Main pearly-white pulse
    draw.line(points, fill=WHITE, width=line_w, joint="curve")

    # Crystalline cyan dot accent at the rightmost peak
    dot_r = max(3, int(inner * 0.055))
    draw.ellipse(
        [(p5[0] - dot_r, p5[1] - dot_r), (p5[0] + dot_r, p5[1] + dot_r)],
        fill=PEARL_CYAN,
        outline=WHITE,
        width=max(1, line_w // 4),
    )


def make_standard(size, path):
    """Standard icon: rounded pomegranate gradient with crystalline highlight + logo."""
...
def make_maskable(size, path):
    """Maskable icon: full-bleed pomegranate gradient, logo inside safe-area (80% center)."""
    bg = _vertical_gradient(size, POM_TOP, POM_BOT)
    canvas = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    canvas.paste(bg, (0, 0))
    _add_crystalline_highlight(canvas, size)
...
def make_apple(size, path):
    """Apple touch icon: no transparency, solid pomegranate w/ logo."""
    bg = _vertical_gradient(size, POM_TOP, POM_BOT)
