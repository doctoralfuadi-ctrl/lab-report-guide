      ...
      <div className="rounded-2xl bg-slate-900/95 backdrop-blur-xl text-white shadow-2xl shadow-cyan-500/20 ring-1 ring-cyan-500/30 overflow-hidden">
        <button type="button" onClick={() => setOpen(v => !v)} className="w-full px-4 py-3 flex items-center gap-3 hover:bg-white/5 transition" data-testid="demo-switcher-toggle">
          ...
          <div className="flex-1 text-start">
            <div className="text-[10px] font-bold tracking-widest uppercase text-cyan-300">{isAr ? "\u0648\u0636\u0639 \u062a\u062c\u0631\u0628\u0629 \u0627\u0644\u0645\u0627\u0644\u0643" : "Owner Demo Mode"}</div>
            <div className="text-sm font-display font-bold">{isAr ? "\u062c\u0631\u0651\u0628 \u0623\u064a\u0651 \u0628\u0627\u0642\u0629 \u0628\u062f\u0648\u0646 \u062f\u0641\u0639" : "Try any tier \u2014 no payment"}</div>
          </div>
          {open ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronUp className="w-4 h-4 text-slate-400" />}
          {onClose && <span role="button" tabIndex={0} onClick={(e) => { e.stopPropagation(); onClose(); }} className="w-7 h-7 rounded-lg hover:bg-rose-500/20 hover:text-rose-300 text-slate-400 flex items-center justify-center transition cursor-pointer" data-testid="demo-switcher-close"><X className="w-3.5 h-3.5" /></span>}
        </button>
        <AnimatePresence initial={false}>
          {open && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
              <div className="px-3 pb-3 space-y-2">
                {tiers.map((t) => { const active = plan === t.key; const Icon = t.Icon; return (
                  <button key={t.key} type="button" onClick={() => setPlan(t.key)} className={`w-full text-start rounded-xl px-3 py-2.5 flex items-center gap-3 transition ${active ? `bg-gradient-to-br ${t.accent} text-white shadow-lg ring-1 ${t.ring}` : "bg-white/5 hover:bg-white/10 text-slate-200"}`} data-testid={`demo-tier-${t.key}`}>
                    <Icon className="w-4 h-4 flex-shrink-0" />
                    <div className="flex-1 min-w-0"><div className="text-sm font-display font-bold leading-tight">{isAr ? t.labelAr : t.labelEn}</div><div className={`text-[11px] tabular-nums ${active ? "text-white/85" : "text-slate-400"}`}>{isAr ? t.priceAr : t.priceEn}</div></div>
