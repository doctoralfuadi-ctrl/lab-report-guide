...
export default function AnalyzerModal({ kind, T, audience, language, api, plan, onClose, onRequestUpgrade }) {
  ...
  // Premium kinds accept BOTH image and PDF. Radiology/ECG accept image+PDF. "image" / "pdf" restrict to that type.
      ...
      {!result && (
        <>
          <div
            ...
            className="border-2 border-dashed border-brand-300 rounded-3xl p-10 text-center bg-brand-50/40 hover:bg-brand-50 cursor-pointer transition"
          ...
          >
            ...
            <p className="text-slate-900 font-semibold">
              {file ? file.name : (kind === "image" ? T.inputs.imageBtn : (isImagingKind || isPremiumKind ? (T.analyzer?.uploadBtn || "Choose image or PDF") : T.inputs.pdfBtn))}
            </p>
            <p className="text-xs text-slate-500 mt-1">
              {file ? `${(file.size / 1024).toFixed(0)} KB` : "Drag & drop or click"}
            </p>
            <input
              ref={inputRef}
              type="file"
          ...
          <div className="mt-6 flex flex-wrap gap-3 justify-end">
            <Button variant="outline" onClick={onClose} className="rounded-full" data-testid="analyzer-cancel">
            ...
            >
              {loading ? (
                ...
                <><Sparkles className="w-4 h-4 me-2" /> {T.nav.analyze}</>
              )}
            </Button>
          </div>
        </>
      ...
      {result && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} data-testid="analyzer-result">
          ...
          <div className="mt-6 flex flex-wrap gap-3 justify-end">
            ...
            <Button onClick={download} className="rounded-full bg-slate-900 hover:bg-brand-600 text-white" data-testid="analyzer-download">
