    ...
    <div className="p-6 max-h-[80vh] overflow-y-auto" data-testid={`analyzer-${kind}`}>
      {!result && (<>
        <div onClick={() => inputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) setFile(f); }} className="border-2 border-dashed border-brand-300 rounded-3xl p-10 text-center bg-brand-50/40 hover:bg-brand-50 cursor-pointer transition" data-testid="dropzone">
          <div className="w-16 h-16 rounded-2xl bg-white border border-brand-100 mx-auto mb-4 flex items-center justify-center shadow-sm">{(kind === "image" || isImagingKind || isPremiumKind) ? <ImageIcon className="w-7 h-7 text-brand-600" /> : <FileText className="w-7 h-7 text-brand-600" />}</div>
        ...
        <div className="mt-6 flex flex-wrap gap-3 justify-end">
          ...
          <Button onClick={handleAnalyze} disabled={!file || loading} className="rounded-full bg-brand-500 hover:bg-brand-600 text-white px-6 h-11" data-testid="analyzer-submit">{loading ? (<><Loader2 className="w-4 h-4 me-2 animate-spin" /> {T.analyzer.analyzing}</>) : (<><Sparkles className="w-4 h-4 me-2" /> {T.nav.analyze}</>)}</Button>
        </div>
      </>)}
      {result && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} data-testid="analyzer-result"><div className="flex items-center gap-2 mb-4"><div className="w-9 h-9 rounded-xl bg-brand-50 flex items-center justify-center"><Sparkles className="w-4 h-4 text-brand-600" /></div><h4 className="font-bold text-lg text-slate-900">{T.analyzer.result}</h4></div><ResultDisplay interpretation={result.interpretation} T={T} plan={plan} onRequestUpgrade={onRequestUpgrade} /><div className="mt-6 flex flex-wrap gap-3 justify-end"><Button variant="outline" onClick={reset} className="rounded-full" data-testid="analyzer-new">{T.analyzer.new}</Button><Button onClick={download} className="rounded-full bg-slate-900 hover:bg-brand-600 text-white" data-testid="analyzer-download"><Download className="w-4 h-4 me-2" /> {T.analyzer.download}</Button></div></motion.div>)}
    </div>
  );
}
