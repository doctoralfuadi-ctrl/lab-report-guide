...
export default function LanguageSelector({ value, onChange, loading, progress }) {
  ...
  const grouped = useMemo(() => {
    ...
    if (!q) {
      // De-duplicate codes across groups (Suggested may overlap)
      const seen = new Set();
      return LANGUAGE_GROUPS.map(g => ({
        label: g.label,
        items: g.codes
          .map(c => LANG_BY_CODE[c])
          .filter(l => l && (g.label === "Suggested" || !seen.has(l.code)))
          .map(l => { if (g.label !== "Suggested") seen.add(l.code); return l; }),
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="inline-flex items-center gap-2 rounded-full border border-brand-200 bg-white/70 backdrop-blur-sm px-3 h-10 text-sm font-medium text-slate-800 hover:bg-white/90 hover:border-brand-400 transition shadow-sm"
      ...
      >
        ...
        <span className="hidden sm:inline" data-testid="language-selector-current">
          {current.label}
        </span>
        <span className="sm:hidden font-bold uppercase text-xs tracking-wider">
          {current.code.split("-")[0]}
        </span>
        {loading && (
          <span className="hidden md:inline text-[10px] text-brand-700 tabular-nums">
      ...
      <DropdownMenuContent
        align="end"
        sideOffset={8}
        className="w-80 max-h-[480px] rounded-2xl border border-brand-100 bg-white/95 backdrop-blur-xl shadow-2xl shadow-brand-900/10 p-0 overflow-hidden"
      ...
      >
        ...
        <div className="max-h-[400px] overflow-y-auto px-2 py-2">
          {grouped.length === 0 && (
            <div className="text-center text-xs text-slate-500 py-6">No language matches &ldquo;{query}&rdquo;.</div>
          )}
          {grouped.map((g) => (
            <div key={g.label} className="mb-2">
              <div className="px-3 pt-1.5 pb-1 text-[10px] font-bold uppercase tracking-widest text-brand-700/70">
                {g.label}
              </div>
              <div className="grid grid-cols-2 gap-0.5">
                {g.items.map((l) => {
                  ...
                  return (
                    <button
                      key={l.code + "_" + g.label}
                      onClick={() => onChange(l.code)}
                      className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-start transition ${
                        active
                    ...
                    >
                      <span className={`text-[10px] uppercase tabular-nums font-mono ${active ? "text-brand-700" : "text-slate-400"}`}>
                        {l.code.split("-")[0]}
                      </span>
