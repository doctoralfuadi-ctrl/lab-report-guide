  ...
  const grouped = useMemo(() => {
    ...
    if (!q) {
      ...
      return LANGUAGE_GROUPS.map(g => ({
        label: g.label,
        items: g.codes.map(c => LANG_BY_CODE[c]).filter(l => l && (g.label === "Suggested" || !seen.has(l.code))).map(l => { if (g.label !== "Suggested") seen.add(l.code); return l; }),
      ...
      <DropdownMenuContent align="end" sideOffset={8} className="w-80 max-h-[480px] rounded-2xl border border-brand-100 bg-white/95 backdrop-blur-xl shadow-2xl shadow-brand-900/10 p-0 overflow-hidden" data-testid="language-selector-content">
        <div className="px-3 pt-3 pb-2 border-b border-brand-50 sticky top-0 bg-white/95 backdrop-blur z-10">
          <div className="relative">
            ...
            <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search 70+ languages…" className="w-full h-9 rounded-full bg-slate-50 ps-9 pe-3 text-sm border border-transparent focus:border-brand-300 focus:bg-white focus:outline-none transition" data-testid="language-selector-search" />
          </div>
        </div>
        <div className="max-h-[400px] overflow-y-auto px-2 py-2">
          ...
          {grouped.map((g) => (
            <div key={g.label} className="mb-2">
              <div className="px-3 pt-1.5 pb-1 text-[10px] font-bold uppercase tracking-widest text-brand-700/70">{g.label}</div>
              <div className="grid grid-cols-2 gap-0.5">
                {g.items.map((l) => {
                  const active = l.code === value;
                  return (
                    <button key={l.code + "_" + g.label} onClick={() => onChange(l.code)} className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-start transition ${active ? "bg-brand-50 text-brand-800 font-semibold ring-1 ring-brand-200" : "hover:bg-slate-50 text-slate-800"}`} data-testid={`language-option-${l.code}`}>
                      <span className={`text-[10px] uppercase tabular-nums font-mono ${active ? "text-brand-700" : "text-slate-400"}`}>{l.code.split("-")[0]}</span>
        ...
        {loading && (
          <div className="border-t border-brand-50 bg-brand-50/50 px-4 py-2.5 flex items-center gap-2 text-xs text-brand-800">
            <Loader2 className="w-3.5 h-3.5 animate-spin" /><span>Translating UI… {progress?.done}/{progress?.total}</span>
          </div>
        )}
      </DropdownMenuContent>
    </DropdownMenu>
